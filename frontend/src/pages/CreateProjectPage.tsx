...
import { runYellowMultiPartySession } from "../lib/yellowSession";
...
const CreateProjectPage = () => {
  ...
  const [yellowRunning, setYellowRunning] = useState(false);
  ...
  const appendYellowLog = useCallback((line: string) => {
    setYellowLogs((prev) => {
  ...
  const runYellowDemo = useCallback(async () => {
    setYellowError(null);
    setYellowLogs([]);
    setYellowRunning(true);
    try {
      await runYellowMultiPartySession({ onLog: appendYellowLog });
    } catch (err) {
      const message =
        err instanceof Error ? err.message : "Yellow session failed";
      setYellowError(message);
      appendYellowLog(`Error: ${message}`);
    } finally {
      setYellowRunning(false);
    }
  }, [appendYellowLog]);
    ...
    <div className="card-frame rounded-lg p-6">
      ...
      <div className="mb-6 rounded border-4 border-dirt p-4 text-xs">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          <p className="text-stone-300">Yellow multi-party session demo</p>
          <button
            type="button"
            onClick={runYellowDemo}
            className="button-blocky rounded px-3 py-1"
            disabled={yellowRunning}
          >
            {yellowRunning ? "Running..." : "Run demo"}
          </button>
        </div>
        <p className="text-[10px] text-stone-400">
          Uses `VITE_WALLET_1_SEED_PHRASE` and `VITE_WALLET_2_SEED_PHRASE` to create,
          update, and close a session.
        </p>
        {yellowError && (
          <p className="mt-2 text-[10px] text-amber-300">{yellowError}</p>
        )}
        <div className="mt-3 max-h-40 overflow-y-auto rounded border-2 border-dirt/70 p-3 text-[11px] text-stone-200">
          {yellowLogs.length === 0 ? (
            <p className="text-[10px] text-stone-400">No logs yet.</p>
          ) : (
            yellowLogs.map((line, idx) => <p key={`${line}-${idx}`}>{line}</p>)
          )}
        </div>
      </div>
